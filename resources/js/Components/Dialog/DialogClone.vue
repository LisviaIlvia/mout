	...
	methods: {
		initialState() {
			return {
				...
				text: this.$tooltipDialog.clone(this.dialogTitle, this.dialogType),
				crudDialog: new this.$crudDialog('clone')
			};
		},
		resetState() {
		...
		openDialog(data, urlForm) {
			...
			this.text.content = "Vuoi clonare "+this.textRequest+" "+data+"?";
			this.crudDialog.open(urlForm, () => {
				this.setup.dialog = true;
				this.$emit('dialog-clone-opened');
			});
		},
		sendForm() {
			this.crudDialog.send(this.form, (data) => {
				this.resetState();
				this.$emit('clone-record', data.record);
				let text = 'clonato';
				if(this.tooltip.type == 'a') text = 'clonato';
