	...
	methods: {
		initialState() {
			return {
				...
				text: this.$tooltipDialog.create(this.dialogTitle, this.dialogType)
			};
		},
		resetState() {
		...
		openDialogCreate(urlOpen, urlForm, callback = null, options = {}) {
			this.crudDialog.urlOpen = urlOpen;
			this.crudDialog.open(urlForm, (data) => {
				if(callback) callback(data);
				this.setup.dialog = true;
				this.$emit('dialog-create-opened');
			},options);
		},
		sendFormCreate(form, options = {}) {
			const {
				field = null, 
				nameField = null, 
				callbackError = null,
			...
			this.crudDialog.send(form, (data) => {
				...
				if (field && typeof field === "object" && 'name' in field && 'value' in field) {
					this.$emit('store-record', data.record, field.name, field.value);
				} else {
					this.$emit('store-record', data.record);
				}
				let recordName = nameField ? data.record[nameField] : data.record.nome;
				let title = this.dialogTitle ? this.dialogTitle : null;
				this.$emit('show-notification', {
					type: 'success',
					text: `${title} ${recordName} creato con successo.`,
				});
			}, {
